    ...
    public partial class ListboxItem : UserControl
    {
        ...
        public ListboxItem(string url,string fileName,string fileId,frmMain formfather,string paperType,string copies,string printMode,string remark,string ID,string Names,string Phone,string upload_time,string send_time,Color color,string user_id,string user_class)
        {
            ...
            if (File.Exists(filePath + "\\" + ID + fileName))
            {
                ...
                //Thread temp = new Thread(new ThreadStart(preparePpt));
                //temp.Start();
            }
            /*************************************/
            ...
            else if (fileName.EndsWith("rar") || fileName.EndsWith("zip"))
            {
                ...
                fileImage = global::SchoolPrint.Properties.Resources.zip;
            }
            else if (fileName.EndsWith("png") || fileName.EndsWith("jpg") || fileName.EndsWith("jpge") || fileName.EndsWith("bmp"))
            {
                fileType = FileType.PICTURE;
                fileImage = global::SchoolPrint.Properties.Resources.picture;
            }
            else
            {
        ...
        public void ListboxItem_Start(object sender, EventArgs e)
        {
            if ((downState != DownloadState.Downloading) && (downState != DownloadState.Finished))
            {
                ...
                //url = "http://www.xiaoyintong.com/" + url;
                //url = "http://api.xiaoyintong.dev:8000/api/v1/desktop/" + url;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (btnDelete.Text == "确认")
            {
                ...
                {
                    MessageBox.Show("请填入正确的金额!");
                    return;
                }
                ChangeOrderMoney(tryMoney);
//                //将金额传到服务器
...
//                }
        ...
        private double LastMoney = 0.0;

        private void ChangeOrderMoney(double money)
        {
            formFather.ChangeOrderMoney(LastMoney, money, ID, user_class);
            LastMoney = money;
            lblDownSpeed.Text = "打印完成";
            lblDownSpeed.ForeColor = Color.Red;
        }
        ...
        private void btnOpen_Click(object sender, EventArgs e)
        {
            if (btnOpen.Text == "下载")
            {
                if (formFather.CheckOrder(ID, user_class))
                    ListboxItem_Start(sender, e);
                else
                    formFather.DeleteOrder(ID, user_class, fileId, Names);
            }
        ...
        private void preparePpt()
        {
            ...
            AutoPrintParameter trans = new AutoPrintParameter();

            trans.fileType = fileType;

            int place, lastPlace;
            place = paperType.IndexOf(',');
            string paperCopiesString = paperType.Substring(0, place);
            paperCopiesString = paperCopiesString.Substring(0, paperCopiesString.LastIndexOf('份'));
            trans.copies = int.Parse(paperCopiesString);

            lastPlace = place;
            place = paperType.IndexOf(',', place + 1);
            trans.paperSize = paperType.Substring(lastPlace + 1, place - lastPlace - 1).Trim();

            lastPlace = place;
            place = paperType.IndexOf(',', place + 1);
            trans.isDuplex = paperType.Substring(lastPlace + 1, place - lastPlace - 1).Trim() == "双面" ? true : false;

            lastPlace = place;
            place = paperType.IndexOf(',', place + 1);
            trans.isBlackWhite = paperType.Substring(lastPlace + 1, place - lastPlace - 1).Trim() == "黑白" ? true : false;

            lastPlace = place;
            place = paperType.IndexOf(',', place + 1);
            string numberPerPageString = paperType.Substring(lastPlace + 1, place - lastPlace - 1).Trim();
            int temp = numberPerPageString.IndexOf('面') + 1;
            numberPerPageString = numberPerPageString.Substring(temp, numberPerPageString.LastIndexOf("个") - temp);
            trans.numberPerPage = int.Parse(numberPerPageString);

            trans.openFilePath = filePath + "\\" + ID + fileName;
            temps = new AutoFilePrint(trans, this);
            temps.preHandlePpt(trans.openFilePath);
            try
            {
                lblDownSpeed.Text = "处理完成，可自动打印";
            }
            catch
            {
                ;
        ...
        private void AutoPrint_Click(object sender, EventArgs e)
        {
            string fileType = fileName.Substring(fileName.LastIndexOf('.') + 1);
            fileType = fileType.ToLower();
            if (!((fileType == "ppt") || (fileType == "pptx")||(fileType == "pdf")))
            {
                MessageBox.Show("暂时只支持ppt和pdf的自动打印");
                return;
            }
            if (btnOpen.Text == "下载")
            {
                MessageBox.Show("请先下载文件！");
                return;
            }
            //if (lblDownSpeed.Text == "下载完成,正在处理...")
            //{
            //    MessageBox.Show("请等待预处理完成后再打印！");
            //    return;
            //}
            if (temps == null)
            {
                ...
                temps.Show();
                if (fileType == "pdf")
                    temps.preHandlePdf(trans.openFilePath);
                else
                    temps.preHandlePpt(trans.openFilePath);
                temps.PrintFile();
            }
        ...
        public void setMoney(string money,string tempfile)
        {
            textBox1.Text = money;
            lblDownSpeed.Text = "打印完成";
            lblDownSpeed.ForeColor = Color.Red;
            ChangeOrderMoney(double.Parse(money));
        }
        ...
        private void textBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                double money;
                if (!double.TryParse(textBox1.Text, out money))
                    MessageBox.Show("请输入正确的金额!");
                else
                    ChangeOrderMoney(money);
            }
        }
    ...
    {
        EXCEL,PDF,PPT,WORD,ZIP,PICTURE,UNKNOW
    }
}
