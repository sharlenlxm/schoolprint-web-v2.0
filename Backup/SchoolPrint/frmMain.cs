                ...
                try
                {
                    ...
                    if (jsonText != "[]")
                    {
                        ...
                        foreach (Document row in result)
                        {
                            ...
                            Remark[k] = row.message;
                            Phone[k] = row.Phone;
                            Names[k] = row.user;
                            upload_time[k] = row.upload_time;
                            send_time[k] = row.send_time;
                            ...
                            department[k] = row.loc.Substring(0, 2);
                            building[k] = Convert.ToInt32(row.loc.Substring(row.loc.IndexOf(" ") + 1, row.loc.IndexOf("栋") - row.loc.IndexOf(" ") - 1));
                            try
                            {
                                room_num[k] = Convert.ToInt32(row.loc.Substring(row.loc.IndexOf("栋") + 2, row.loc.IndexOf("室") - row.loc.IndexOf('栋') - 2));
                            }
                            catch
                            {
                                MessageBox.Show(row.loc + "|" + (row.loc.IndexOf("室") - row.loc.IndexOf('栋')));
                            }
                        ...
                        MessageBox.Show("共" + result.Count.ToString() + "个打印任务");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (button1.Text == "登录系统")
                {
                    ...
                    string ShoperIds = "l_username";
                    string Passwords = "l_pwd";
...
#region vip user name
        ...
        public double AllMoney(string building)
        {
            double moneyCount = 0.0;
            double tempMoney=0.0;
            System.Windows.Forms.Control[] temp = this.panel1.Controls.Find(building, true);
